...
export function Login() {
  ...
  const [errorKey, setErrorKey] = useState(0); // forces the shake animation to replay on repeated errors
  ...
  return (
    <div className="auth-container-wrapper">
      ...
      <style>{`
        ...
        .auth-card { width: 100%; max-width: 420px; background: #ffffff; border: 1px solid #e2e0d8; border-radius: 20px; padding: 40px 40px 36px; box-shadow: 0 4px 40px rgba(15,25,35,0.07); text-align: left; box-sizing: border-box; animation: fadeSlideUp 0.55s cubic-bezier(0.22, 1, 0.36, 1) 0.1s both; }

        @keyframes fadeSlideUp { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes shake { 10%, 90% { transform: translateX(-1px); } 20%, 80% { transform: translateX(2px); } 30%, 50%, 70% { transform: translateX(-4px); } 40%, 60% { transform: translateX(4px); } }
        @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        @keyframes pulse { 0%, 100% { opacity: 1; box-shadow: 0 0 0 0 rgba(29,158,117,0.5); } 50% { opacity: 0.7; box-shadow: 0 0 0 4px rgba(29,158,117,0); } }
        ...
        .auth-tag span { width: 6px; height: 6px; border-radius: 50%; background: #1d9e75; display: inline-block; animation: pulse 2s ease-in-out infinite; }
        ...
        .field input { width: 100%; height: 44px; border: 1px solid #e2e0d8; border-radius: 10px; background: #ffffff; padding: 0 14px; font-family: 'DM Sans', sans-serif; font-size: 14px; color: #0f1923; outline: none; transition: border 0.2s, box-shadow 0.2s, transform 0.15s; box-sizing: border-box; }
        .field input:focus { border-color: #0f6e56; box-shadow: 0 0 0 3px rgba(15,110,86,0.1); transform: translateY(-1px); }
        .field-icon { position: relative; }
        .field-icon input { padding-left: 40px; }
        .field-icon i { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); font-size: 16px; color: #c4c2bc; pointer-events: none; transition: color 0.2s; }
        .field-icon input:focus ~ i { color: #0f6e56; }

        .btn-primary { width: 100%; height: 48px; background: #0f1923; color: #fff; border: none; border-radius: 11px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 600; letter-spacing: 0.3px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; transition: background 0.2s, transform 0.12s; margin-top: 8px; box-sizing: border-box; position: relative; overflow: hidden; }
        .btn-primary::before { content: ''; position: absolute; top: 0; left: -100%; width: 60%; height: 100%; background: linear-gradient(120deg, transparent, rgba(255,255,255,0.08), transparent); transition: left 0.5s ease; }
        .btn-primary:hover:not(:disabled)::before { left: 130%; }
        .btn-primary:hover:not(:disabled) { background: #1a2d3e; transform: translateY(-1px); }
        ...
        .auth-switch a { color: #0f6e56; font-weight: 600; text-decoration: none; cursor: pointer; transition: color 0.15s; }
        .auth-switch a:hover { text-decoration: underline; color: #0a4a3b; }
        ...
        .error-message { background: #fdf2f2; color: #9b1c1c; border: 1px solid #f8b4b4; padding: 10px 12px; border-radius: 8px; font-size: 13px; margin-bottom: 16px; font-weight: 500; animation: shake 0.4s ease-in-out; }
      ...
      <div className="auth-shell">
        ...
        <div className="auth-main">
          <div className="auth-card">
            ...
            {error && <div className="error-message" key={errorKey}>{error}</div>}
              ...
              <button type="submit" className="btn-primary" disabled={loading}>
                {loading ? (
                  ...
                  <>
                    ...
                    <svg className="btn-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: '16px', height: '16px' }}>
